...
export default function NgfEditBridge() {
  useEffect(() => {
    ...
    style.textContent = `
      ...
      #ngf-nav-popup .ngf-stay-btn { color: #6b7280; background: transparent; }
      #ngf-nav-popup .ngf-stay-btn:hover { background: #f3f4f6; }
    `
    ...
    function showNavPopup(href: string, label: string, clientX: number, clientY: number) {
      ...
      stayBtn.addEventListener('click', ev => { ev.stopPropagation(); dismissNavPopup() })
        ...
        function applyFlat(obj: Record<string, unknown>, prefix: string) {
          for (const [key, value] of Object.entries(obj)) {
            ...
            if (typeof value === 'string') {
              ...
              value.forEach((item, i) => { if (item && typeof item === 'object') applyFlat(item as Record<string, unknown>, `${path}.${i}`) })
    ...
    const clickHandler = (e: MouseEvent) => {
      ...
      while (target && target !== document.documentElement) {
        ...
        if (attr) {
          ...
          if (dot > -1) {
            ...
            window.parent.postMessage({ type: 'fieldClick', section: attr.substring(0, dot), field: attr.substring(dot + 1), currentValue: target.textContent?.trim() ?? '', elementRect: { top: rect.top, left: rect.left, bottom: rect.bottom, right: rect.right, width: rect.width, height: rect.height } }, '*')
          }
          return
        }
      ...
      while (target && target !== document.documentElement) {
        ...
        if (tag === 'a') {
          ...
          else if (href && href !== '#') { showNavPopup(anchor.href, anchor.textContent?.trim() || 'Link', e.clientX, e.clientY) }
          return
        }
